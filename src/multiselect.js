...
;(function (factory) {
    ...
    $.fn.move = function( $options ) {
        var thisVal = this;
        thisVal.prop('selected', false).find('option');
        var array_ = [];
        if('customSort' == thisVal.attr('id')){
            $options.remove();            
        }else{
            $('#customSort_to > option').each(function() {
                array_.push($(this).val());
            });
            $('#customSort > option').each(function() {
                $(this).prop('selected', false);
            });
            if(array_.length == 0){
                thisVal.append($options.clone());
            }else{
                if(array_.indexOf($options.val()) > -1){}else{
                    thisVal.append($options.clone());
                }
            }            
        }       
